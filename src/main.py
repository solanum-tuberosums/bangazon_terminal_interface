        ...
        if menu_command is None:
                ...
                if active_customer_id:
                    ...
                    if len(title) <= 0:
                        ...
                        product_values.extend(
                            [price, title, description, product_type_id,
                            customer_id]
                            )
                        save_to_db("Product", product_values)
                        print("\n *** PRODUCT SAVED *** \n")
                        menu_command = None
            else:
                input('Please select an active customer or create a new '
            ...
            if active_customer_id:
                ...
                for product in details:
                    print(str(i) + ". " + product[5] + ": $" + str(product[4]))
                    i += 1
                order_total = get_order_total(order_id)
                print("**************************************")                
                print("ORDER TOTAL: ${}".format(round(order_total, 2)))
                print("\nPRESS ENTER TO RETURN TO THE MAIN MENU")
                input(" > ")
                menu_command = None
            else:
